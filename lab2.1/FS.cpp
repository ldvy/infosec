...
void Filesystem::defineUserInfo(unsigned int blockAddress, string username) {  //.pw data structure username:password:homedir:isblocked(0 / 1):number of remaining failed login attempts:Q1:A1:Q2:A2:Q3:A3
	// PLUS username
	...
	if (username == "guest")
		...
		if (username == "root")
			...
			// PLUS isBlocked (0 = false) AND failed question attempts
			content += ":0:" + to_string(INITIAL_FAILED_ATTEMPTS);
...
bool Filesystem::deleteUser(string username) {
	if (deleteFile("/secrets/" + username + ".pw") and deleteFile("/home/" + username))
		return true;
	else
		return false;
}