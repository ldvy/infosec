...
const int COMMANDS_LEFT = 10;  // Every 10 commands for the user a question is asked
const int USER_SCORE = 5;  // Every failed quesion results in -1 points.
const int KICKS_LEFT = 3; // Number of times the user needs to be kicked before he gets banned
...
public:
	...
	vector<string> getUserInfo(string username);
	...
	bool notifyRoot(string path);

	void clearFile(unsigned int blockAddress);

	void userOperations(string username, int operation);

	void modifyUserData(string username, int position, int scoreIncrement);

	bool askQuestion(string username);
...
private:
	...
	bool createFile(unsigned int& inodeNum, unsigned int size = 0, bool edit = false, string username = "");

	void defineUserInfo(unsigned int blockAddress, string username);
	...
	bool addDirItem(unsigned parentDirInodeNum, unsigned fileInodeNum,
		const string& name);
};
