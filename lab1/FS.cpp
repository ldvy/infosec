...
bool Filesystem::createUser(string username) {
	...
	string fileName = names[1];
	unsigned int inodeNum;
	if (createUserFile(inodeNum, 1024, true, username)) {
		return addDirItem(parentDirInodeNum, inodeNum, fileName);
	}
...
bool Filesystem::createUserFile(unsigned int& inodeNum, unsigned int size, bool edit, string username) {
	if (assignInode(inodeNum)) {
		Inode inode = createInode(false);
		inode.size = size;
		int blockNum = size / BLOCK_SIZE;
...
void Filesystem::defineUserInfo(unsigned int blockAddress, string username) {
	...
	cout << "Define root password:\n";
	while (infoPart == "") {
		getline(cin, infoPart);
	}

	if (username == "root")
		content += ":" + infoPart + ":" + "/";
	else
		content += ":" + infoPart + ":" + "/home/" + username;
...
bool Filesystem::checkDirPermissions(string username) {
	if (workingDir.find(getUserInfo(username)[2]) != std::string::npos)
		return true;
	else
		return false;
}
